...
namespace SiPV.AssetLoader.Samples.Demo
{
    ...
    public sealed class DemoController : MonoBehaviour
    {
        ...
        [Header("Cache tuning")]
        [Tooltip("Max number of entries the RAM cache holds before evicting the least recently used one.")]
        [SerializeField] private int _ramMaxEntries = 32;

        [Tooltip("Max total size, in megabytes, the RAM cache is allowed to hold.")]
        [SerializeField] private int _ramMaxBytesMb = 32;

        [Tooltip("Max total size, in megabytes, the disk cache is allowed to hold before trimming old entries.")]
        [SerializeField] private int _diskBudgetMb = 64;

        [Header("Request policy tuning")]
        [Tooltip("Timeout applied to each individual fetch attempt, in seconds.")]
        [SerializeField] private float _requestTimeoutSeconds = 10f;

        [Tooltip("Max attempts (including the first) before a request gives up, used by DefaultRetryPolicy.")]
        [SerializeField] private int _maxRetryAttempts = 3;

        [Tooltip("Number of concurrent LoadAsync calls the \"Load 5 concurrent\" button fires, to demonstrate dedup coalescing.")]
        [SerializeField] private int _concurrentLoadCount = 5;
        ...
        // (a deliberate omission: the cache doesn't need to answer "what's in you" for the
        // pipeline to work, only this demo wants that view)
        ...
        private void Awake()
        {
            ...
            var diskCache = new FileDiskCache(diskCacheDir, metadataStore, budgetBytes: (long)_diskBudgetMb * 1024 * 1024);
            ...
            _config = new AssetLoaderConfigBuilder()
                ...
                .UseRamCache(new RamCache(new DefaultMemorySizeEstimator(), new DefaultAssetReleaser(), maxEntries: _ramMaxEntries, maxBytes: (long)_ramMaxBytesMb * 1024 * 1024))
                .UseDiskCache(diskCache, metadataStore)
                .UseCacheKeyProvider(_keyProvider)
                .UseTimeoutPolicy(new DefaultTimeoutPolicy(perAttemptTimeout: TimeSpan.FromSeconds(_requestTimeoutSeconds)))
                .UseRetryPolicy(new DefaultRetryPolicy(maxAttempts: _maxRetryAttempts))
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button($"Load {_concurrentLoadCount} concurrent (dedup proof)"))
            {
        ...
        private async UniTaskVoid LoadFiveConcurrentAsync()
        {
            ...
            _status = $"Loading {_concurrentLoadCount} concurrent requests for the same URL...";

            try
            {
                var request = new AssetRequest(_imageUrl);
                var tasks = new UniTask<AssetHandle<Texture2D>>[_concurrentLoadCount];
                ...
                try
                {
                    ...
                    _status = $"{_concurrentLoadCount} concurrent loads finished. HTTP requests made: {after - before} (should be 0 or 1, never {_concurrentLoadCount}) - proves dedup coalescing.";
