...
| **Clear disk cache** | Empties every entry via `IDiskCache.EvictAsync` + `IDiskCacheMetadataStore.RemoveAsync` together (they're separate stores, both need clearing or they drift apart). Click **Load** again after this (and after Clear RAM): full `CacheMiss`, real network fetch. |
...
| **Cache stats line** | RAM entries/bytes are tracked by the demo itself (`IRamCache` intentionally has no stats accessor, so nothing in the package needs to track this). Disk entries/bytes come straight from the real cache via `AssetLoaderConfig.DiskCache`/`MetadataStore`, which the demo holds a direct reference to since it built the config itself. |
