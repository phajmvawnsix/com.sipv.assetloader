...
| **Clear disk cache** | Empties every entry via `IDiskCache.EvictAsync` + `IDiskCacheMetadataStore.RemoveAsync` together (they're separate stores, both need clearing or they drift - see `docs/DesignDraft.txt`). Click **Load** again after this (and after Clear RAM): full `CacheMiss`, real network fetch. |
